...
# multi_qa_embeddings_sentences <- read_csv("~/Google Drive/My Drive/Share_Clients/data_science_project_work/hackafun/data/semantic_search_data/example_multi-qa_embeddings_sentences.csv")
...
## Function 1
cosine_calculation_threshold_sentence(reference_statement = "the terminator", 
                             cosine_sim_threshold = 0.5, 
                             embedding_model = multi_qa_embedder, 
  ...
  mutate(test_text = mapply(highlight_sentences, text_copy, sentences)) %>%
  right_join(example_sentences, by = "document") %>%
  select(text_copy = text_copy.x, sentence, document, test_text)
```
