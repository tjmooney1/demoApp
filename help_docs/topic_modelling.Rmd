...
# Quick edit of Jamie's sentence df and save to repo

```{r}
cosmetic_sentences <- readr::read_csv("~/Google Drive/My Drive/Share_Clients/data_science_project_work/hackafun/data/cleaned_data/for_app/cosmetic_sentences.csv")

cosmetic_sentences_embeddings <- readr::read_csv("~/Google Drive/My Drive/Share_Clients/data_science_project_work/hackafun/data/cleaned_data/for_app/cosmetic_sentences_embeddings_quant.csv")

cosmetic_sentences %>% colnames()

cosmetic_sentences_topics <- cosmetic_sentences %>% 
  dplyr::select(-c("V1", "V2")) %>%
  dplyr::left_join(beauty_topics %>% 
                     dplyr::select(universal_message_id, topic, topic_title, V1, V2),
                   by = "universal_message_id") 
  
cosmetic_sentences_topics %>%
  readr::write_rds("app/data/cosmetic_sentences.rds")

cosmetic_sentences_embeddings %>%
  readr::write_rds("app/data/cosmetic_sentences_embeddings.rds")
```
